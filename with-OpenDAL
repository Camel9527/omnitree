...
```mermaid
graph TB
    App[应用程序] --> LocalDir[本地目录]
    App --> OmnitreeAPI[Omnitree.sync API]
    
    OmnitreeAPI --> SyncEngine[同步引擎]
    
    SyncEngine --> Scanner[本地扫描器]
    SyncEngine --> MetaDB[(元数据数据库)]
    SyncEngine --> OpenDAL[OpenDAL 统一存储层]
    
    Scanner --> LocalDir
    
    OpenDAL --> OneDrive[OneDrive]
    OpenDAL --> GoogleDrive[Google Drive]
    OpenDAL --> iCloud[iCloud Drive]
    OpenDAL --> Others[S3/Dropbox/等40+服务]
    
    style App fill:#e1f5ff
    style LocalDir fill:#fff4e1
    style OpenDAL fill:#f0e1ff
```

**架构说明**：
- 应用直接使用标准文件 API（如 `std::fs`）读写本地目录
- 应用调用 `Omnitree.sync()` 触发同步
- Omnitree 扫描本地目录变化，通过 OpenDAL 与云盘同步
- OpenDAL 提供统一的云存储访问接口

### 2.2 模块划分

#### 2.2.1 核心模块
- **同步引擎**：协调本地与云盘的同步逻辑，实现多设备间的数据一致性，基于时间戳判断最新版本
- **云盘访问层（OpenDAL）**：统一的存储访问接口，支持 OneDrive、Google Drive、iCloud 等 40+ 种存储服务
- **元数据管理**：维护文件状态、修改时间等信息，支持增量同步和冲突检测
- **变更检测**：扫描本地目录，识别新增、修改、删除的文件

**注意**：本库不提供本地文件操作 API，应用直接使用标准文件 API（如 `std::fs`）读写本地目录，库只负责检测变化并同步到云盘。
...
pub struct Omnitree {
    ...
    operator: opendal::Operator,  // OpenDAL 操作符
}
...
impl Omnitree {
    ...
    pub fn new(config: SyncConfig, credentials: &CloudCredentials) -> Result<Self, Error>;

    /// 初始化本地目录和元数据库（首次使用时调用）
    /// 
    /// # 返回
    /// - `Result<(), Error>`: 成功或错误
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 3. 创建库实例
    let mut omnitree = Omnitree::new(config, &credentials)?;

    // 4. 初始化（首次使用）
    omnitree.initialize()?;

    // 5. 首次同步：从云盘下载已有文件
    let result = omnitree.sync().await?;
    println!("首次同步完成:");
    println!("  下载: {} 个文件", result.downloaded);
    println!("  上传: {} 个文件", result.uploaded);
    println!("  耗时: {:?}", result.duration);

    // 6. 应用直接使用标准文件 API 读写本地文件
    let local_root = PathBuf::from("/path/to/local/folder");
    fs::write(local_root.join("notes.txt"), b"Hello, World!")?;
    let content = fs::read_to_string(local_root.join("notes.txt"))?;
    println!("文件内容: {}", content);

    // 7. 再次同步：Omnitree 自动检测本地变化并上传到云盘
    let result2 = omnitree.sync().await?;
...
// 电脑 A：修改文件并同步到云盘
let local_a = PathBuf::from("/Users/alice/MyApp");
let mut omnitree_a = Omnitree::new(config_a, &credentials)?;

// 应用直接写文件
fs::write(local_a.join("document.txt"), b"Updated content")?;

// 同步到云盘
omnitree_a.sync().await?;  // Omnitree 检测到变化并上传

// 电脑 B：从云盘同步最新文件
let local_b = PathBuf::from("/Users/bob/MyApp");
let mut omnitree_b = Omnitree::new(config_b, &credentials)?;

// 同步下载
omnitree_b.sync().await?;  // 下载电脑 A 的更改

// 应用直接读文件
let content = fs::read_to_string(local_b.join("document.txt"))?;
...
let gdrive_creds = CloudCredentials {
    token: "gdrive_token".to_string(),
    provider: CloudProvider::GoogleDrive,
    expires_at: None,
    refresh_token: None,
};

// Windows 用户：使用 OneDrive
let mut omnitree_onedrive = Omnitree::new(config.clone(), &onedrive_creds)?;
let result1 = omnitree_onedrive.sync().await?;
println!("OneDrive 同步完成 - Windows/跨平台设备间同步");

// 使用 Google Drive（得益于 OpenDAL，支持 40+ 种存储服务）
let mut omnitree_gdrive = Omnitree::new(config.clone(), &gdrive_creds)?;
let result2 = omnitree_gdrive.sync().await?;
println!("Google Drive 同步完成 - 跨平台同步");
```
